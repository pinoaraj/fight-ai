...
for (;;) {
  try {
    ...
    const found = await dynamo.send(new ScanCommand({
      TableName: table,
      FilterExpression: '#status IN (:q,:d,:c,:u,:p,:g) AND (attribute_not_exists(leaseExpiresAt) OR leaseExpiresAt < :now OR updatedAt < :stale)',
    ...
    const claimValues = {
      ...scanValues,
      ':owner': { S: owner },
      ':lease': { N: String(now + 720000) },
    };
    await dynamo.send(new UpdateItemCommand({
      TableName: table,
      Key: { jobId: { S: id } },
      UpdateExpression: 'SET leaseOwner = :owner, leaseExpiresAt = :lease, updatedAt = :now',
      ConditionExpression: '#status IN (:q,:d,:c,:u,:p,:g) AND (attribute_not_exists(leaseExpiresAt) OR leaseExpiresAt < :now OR updatedAt < :stale)',
      ExpressionAttributeNames: names,
    ...
    console.error('Fight AI durable worker loop error', error instanceof Error ? error.message : error);
