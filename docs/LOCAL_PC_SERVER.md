...
Usa `VER_ACCESO_EXTERNO.cmd` para mostrar localmente solo el enlace, usuario y contrasena, sin exponer la clave Gemini. Envia el enlace y las credenciales por canales separados. `DETENER_ENLACE_EXTERNO.cmd` detiene solamente el PID de `cloudflared`; el servidor LAN sigue activo. El enlace cambia al reiniciar y es solo para pruebas. El proxy gratuito limita cada solicitud a 100 MB, por lo que videos mayores requieren un futuro flujo local por partes o un servicio permanente adecuado.
