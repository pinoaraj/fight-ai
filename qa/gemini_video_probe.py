...
for _ in range(60):
    state=json.loads(req(base+'/v1beta/'+name+'?key='+urllib.parse.quote(KEY)))['state']
...
out=json.loads(req(base+'/v1beta/models/'+MODEL+':generateContent?key='+urllib.parse.quote(KEY),json.dumps(payload).encode(),'POST',{'Content-Type':'application/json'}))
