...
export default function Home() {
  ...
  function selectVideo(file: File | null) {
    ...
    setVideo(file); setReport(null); setFrames({}); setAnchor(null); setMarking(false); setPreviewReady(false); setPreviewAttempting(Boolean(file)); setPreviewError(file ? 'Comprobando compatibilidad del video…' : ''); setFallbackFrameUrl(''); setSelectionFrameUrl(''); setStagedVideoId(''); setPreviewTime(0); setError(''); setReplayStatus(''); setUploadedSession(null); setFocusConfirmed(false);
    if (file) {
      window.setTimeout(() => document.getElementById('fighter-step')?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 180);
      // Do not leave the user staring at a black HEVC canvas. Start decoding
      // as soon as the preview mounts; decodePreview falls back to FFmpeg/JPEG.
      window.setTimeout(() => {
        const node = videoRef.current;
        if (node) void decodePreview(node, true);
        else void generateCompatibleFrame();
      }, 280);
    }
  }
  ...
  return <main>
    ...
    <section className="workspace" id="analyze"><aside className="panel uploadPanel">
      ...
      {!video ? <button className="drop" data-testid="upload-button" onClick={() => inputRef.current?.click()}><span className="dropKicker">PASO 1 · COMIENZA AQUÍ</span><span className="uploadIcon">↑</span><strong>SUBE 1 ROUND · HASTA 3 MINUTOS</strong><span className="dropCopy">Arrastra tu video aquí o selecciónalo desde tu teléfono</span><small>MP4 o MOV · si es más largo, analizaremos automáticamente 0:00–3:00</small><span className="dropAction">ELEGIR VIDEO <b>→</b></span></button> : <div className="videoWrap" id="fighter-step"><div className="videoTask"><span>PASO 2 · IDENTIFICA A TU ATLETA</span><b>{previewReady?'Ahora marca a tu peleador':'Preparando un frame verificable'}</b></div><div className={`videoStage ${marking?'isMarking':''}`}>{fallbackFrameUrl || selectionFrameUrl ? <img data-testid="compatible-frame" className="compatibleFrame" src={fallbackFrameUrl || selectionFrameUrl} alt="Frame fijo para marcar al peleador"/> : <video data-testid="video-preview" ref={videoRef} src={videoUrl} controls playsInline muted preload="auto" onLoadedMetadata={e=>void decodePreview(e.currentTarget)} onError={()=>{ if (!fallbackFrameUrl && !previewAttempting) void generateCompatibleFrame(); }}/>} {previewAttempting && !fallbackFrameUrl && !selectionFrameUrl && <div className="previewPreparing" data-testid="preview-preparing"><span className="spinner"/><b>PREPARANDO FRAME DEL PELEADOR</b><small>{previewError || 'Comprobando el video y generando una imagen compatible si hace falta.'}</small></div>} {anchor && <div className="fighterCircle" style={{left:`${anchor.x}%`,top:`${anchor.y}%`,width:`${anchor.size}%`,aspectRatio:'1'}}/>}{marking && <div className="markerOverlay" data-testid="marker-overlay" onClick={setAnchorFromEvent}><span>Toca el centro del peleador</span></div>}</div><div className="previewHint" data-testid="preview-status"><b>{previewReady?'AHORA MARCA A TU PELEADOR':previewAttempting?'PREPARANDO FRAME COMPATIBLE…':'FRAME NO DISPONIBLE'}</b><span>{previewReady?'Pulsa “Marcar peleador”: aparecerá una imagen fija sin controles para tocar al atleta.':previewError}</span>{!previewReady && <button data-testid="decode-frame" onClick={()=>void generateCompatibleFrame()} disabled={previewAttempting}>{previewAttempting?'GENERANDO FRAME…':'REINTENTAR FRAME COMPATIBLE'}</button>}</div><div className="fileRow"><div><b>{video.name}</b><span>{(video.size/1024/1024).toFixed(1)} MB · análisis 0:00–3:00 máximo</span></div><button onClick={() => inputRef.current?.click()}>Cambiar</button></div>{replayStatus && <div className="replayStatus">▶ {replayStatus}</div>}</div>}
