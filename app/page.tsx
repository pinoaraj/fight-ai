...
export default function Home() {
  ...
  return <main>
    ...
    <section className="workflowStrip" aria-label="Progreso del análisis">{['Video','Marcar peleador','Características','Foco del coach','Reporte'].map((label,index)=>{ const current=workflowStep===index+1; return <span key={label} className={`${current?'active nextPulse':workflowStep>index+1?'done':''}`}><i>{workflowStep>index+1?'✓':index+1}</i><b>{label}</b>{current && <small>AHORA</small>}</span>; })}</section>
