...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!range) return new NextResponse(bytes, { status: 200, headers: { ...common, 'Content-Length': String(bytes.length) } });
    ...
    return new NextResponse(chunk, { status: 206, headers: { ...common, 'Content-Length': String(chunk.length), 'Content-Range': `bytes ${start}-${end}/${bytes.length}` } });
  } catch (error) {
